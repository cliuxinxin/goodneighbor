    ...
    <div class="row">
        ...
        <div class="col-md-1 col-md-offset-1">
            ...
            @if($task->isNotReceived() && (Auth::guest() || $task->notSendByUser(Auth::user())))
                <a class="btn btn-danger" href="{{ url('tasks/take').'/'.$task->id }}">我要帮忙</a>
            @endif
            ...
            @if(Auth::check() && $task->isCanConfirm(Auth::user()))
                <a class="btn btn-danger" href="{{ url('tasks/confirm').'/'.$task->id }}">确认完成</a>
            @endif
        ...
        <div class="col-md-1">
            @if(Auth::check() && $task->isCanRemove(Auth::user()))
                <a class="btn btn-danger" href="{{ url('tasks/remove').'/'.$task->id }}">取消{{ $task->receiver->name }}的帮助</a>
            @endif
        </div>
